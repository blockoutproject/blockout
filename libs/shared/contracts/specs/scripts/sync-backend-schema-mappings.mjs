...
const backendPom = path.resolve(scriptDir, '../../../../../apps/backend/pom.xml');
...
async function schemaNames() {
  ...
  for (const file of files) {
    names.push(...Object.keys(JSON.parse(await readFile(path.join(sharedSchemasDir, file), 'utf8'))));
  }
...
function mappingBlock(names) {
  return [
    ...
    '                        <schemaMappings>',
    ...names.map(
      (name) => `                            <schemaMapping>${name}=com.blockout.shared.model.${name}</schemaMapping>`,
    ),
    '                        </schemaMappings>',
...
try {
  ...
  if (start === -1 || end < start) {
    throw new Error('Could not find schemaMappings markers in apps/backend/pom.xml');
  }
