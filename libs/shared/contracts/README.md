...
operations or reusable components reference. MRG-317 through MRG-325 make `config.json`, `clubs.json`, `teams.json`,
`pools.json`, `competition.json`, `matches.json`, `users.json`, `reports.json`, and `notification.json` authoritative
owner bundles, with their audited canonical operations and required wire fields. Later owner tasks populate the
remaining service bundles.
