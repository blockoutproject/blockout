...
operations or reusable components reference. MRG-317 through MRG-322 make `config.json`, `clubs.json`, `teams.json`,
`pools.json`, `competition.json`, and `matches.json` authoritative owner bundles, with their audited canonical
operations and required wire fields. Later owner tasks populate the remaining service bundles. Bundle authority
defines the target contract; it does not activate routes or change runtime authority.
