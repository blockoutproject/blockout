...
operations or reusable components reference. MRG-317 and MRG-318 make `config.json` and `clubs.json` authoritative
owner bundles, with their audited canonical operations and required wire fields. Later owner tasks populate the
remaining service bundles. Bundle authority defines the target contract; it does not activate routes or change runtime
authority.
