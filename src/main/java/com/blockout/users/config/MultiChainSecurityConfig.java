...
public class MultiChainSecurityConfig {
    ...
    public SecurityFilterChain apiKeyChain(HttpSecurity http) throws Exception {
        http
            ...
            .securityMatcher(httpRequest -> 
                 HttpMethod.POST.toString().equals(httpRequest.getMethod()) &&
                 httpRequest.getRequestURI().equals("/users/v1/users")
            )
    ...
    public SecurityFilterChain bearerChain(HttpSecurity http) throws Exception {
        http
            // Pas de `securityMatcher` => s'applique à tout ce qui n'a pas été matché par la chaîne précédente
            .csrf(csrf -> csrf.disable())
            .sessionManagement(sm -> sm.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
