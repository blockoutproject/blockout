...
> = ({ match }) => {
    ...
    const links: MatchLiveLinkDTO[] = useMemo(() => data ?? [], [data]);
    ...
    const handleApprove = useCallback(
        async (link: MatchLiveLinkDTO) => {
            try {
                ...
                await mobile.matches.approvePendingLiveLink(link.id);
                await refetch();
    ...
    const handleReject = useCallback(
        async (link: MatchLiveLinkDTO) => {
            try {
                setApiError(null);
                await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
                await mobile.matches.rejectPendingLiveLink(link.id);
                await refetch();
                await Haptics.notificationAsync(
                    Haptics.NotificationFeedbackType.Success,
                ).catch(() => { });
            } catch (err) {
                setApiError("Impossible de rejeter ce lien, réessaie.");
                await Haptics.notificationAsync(
                    Haptics.NotificationFeedbackType.Error,
                ).catch(() => { });
            }
    ...
    const handleDeleteActive = useCallback(
        async (_link: MatchLiveLinkDTO) => {
            try {
                setApiError(null);
                await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
                await mobile.matches.deleteMatchLiveLink(match.id);
                await refetch();
                await Haptics.notificationAsync(
                    Haptics.NotificationFeedbackType.Success,
                ).catch(() => { });
            } catch (err) {
                setApiError("Impossible de supprimer ce lien, réessaie.");
                await Haptics.notificationAsync(
                    Haptics.NotificationFeedbackType.Error,
                ).catch(() => { });
            }
        },
        [mobile, match.id, refetch],
    );
    ...
    const handleReactivate = useCallback(
        async (link: MatchLiveLinkDTO) => {
            try {
                setApiError(null);
                await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
                await mobile.matches.reactivateLiveLink(link.id);
                await refetch();
                await Haptics.notificationAsync(
                    Haptics.NotificationFeedbackType.Success,
                ).catch(() => { });
            } catch (err) {
                setApiError("Impossible de réactiver ce lien, réessaie.");
                await Haptics.notificationAsync(
                    Haptics.NotificationFeedbackType.Error,
                ).catch(() => { });
            }
