...
- Owner: `apps/backend/backend-migrations`; PostgreSQL 17, Liquibase 5.0.3.
...
- Validate with `./mvnw -f apps/backend/pom.xml -pl backend-migrations,backend-jobs,backend-api,backend-worker -am verify`, complete backend/workspace validation and `scripts/backend-foundation/smoke.sh`.
