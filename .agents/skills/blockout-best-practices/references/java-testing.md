...
Handwritten test classes and non-obvious shared test helpers follow `code-documentation.md`. Keep one-off test
...
- Follow `code-documentation.md` for testkits and non-obvious test contracts.
...
- Run the complete backend reactor selected by the skill entry point and repository build configuration when a shared transport, parent build,
