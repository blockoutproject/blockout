...
import { deleteMobileNotification } from '@/src/api/generated/mobile-gateway/endpoints/mobile-notifications/mobile-notifications';
import {
  DeleteMobileNotificationParams,
  DeleteMobileNotificationResponse,
} from '@/src/api/generated/mobile-gateway/schemas/mobile-notifications/mobile-notifications.zod';

const NOTIFS_QK = (pageSize: number) =>
...
export function useDeleteNotification(pageSize = 20) {
  ...
  return useMutation({
    mutationFn: async (id: number) => {
      const path = DeleteMobileNotificationParams.parse({ id });
      const response = await deleteMobileNotification(path.id);
      DeleteMobileNotificationResponse.parse(response);
    },
    onMutate: async (id: number) => {
      ...
      const previous =
        qc.getQueryData<InfiniteData<EnrichedUserNotificationPage>>(key);

      // Maj optimiste : on retire l’item de toutes les pages
      qc.setQueryData<InfiniteData<EnrichedUserNotificationPage>>(
        key,
        (old) => {
          if (!old) return old;
          return {
