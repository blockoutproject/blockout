...
export type EntityTabViewProps = {
  tabs: TabDefinition[];
  scrollYs: Record<string, Animated.Value>;
  onTabChange?: (key: string) => void;
};

const EntityTabView: React.FC<EntityTabViewProps> = ({
  tabs,
  scrollYs,
  onTabChange,
}) => {
  ...
  const renderLazyPlaceholder = ({ route }: { route: Route }) => {
    const tabDef = tabs.find((t) => t.key === route.key);
    return tabDef?.renderPlaceholder ? (
      tabDef.renderPlaceholder()
    ) : (
      <View style={{ flex: 1 }} />
    );
  };
  ...
  const renderTabBar = (
    ...
    const interpolatedOpacity = routes.reduce<
