...
import RawDivisionMappingFormSheet from "@/src/modules/raw-division-mapping/ui/raw-division-mapping-form-sheet";
...
const RawDivisionMappingScreen: React.FC = () => {
  ...
  const [editing, setEditing] = useState<RawDivisionMappingResponse | null>(
    null,
  );
  ...
  const seasonNames = useMemo<string[]>(
    () =>
      Array.from(new Set((data ?? []).map((d) => String(d.season)))).sort(
  ...
  const openForm = (mapping: RawDivisionMappingResponse) => {
    Keyboard.dismiss();
  ...
  const activeLeagues = leagueFilters
    .filter((f) => f.isActive)
    .map((f) => f.name);
  const activeSeasons = seasonFilters
    .filter((f) => f.isActive)
    .map((f) => f.name);
  ...
  if (isLoading || !data) {
    return (
      <View
        ...
        testID="raw-division-mapping-loading"
      >
  ...
  return (
    <>
      <View
        ...
        testID="raw-division-mapping-screen"
      >
        <SearchField
          value={search}
        ...
        <FlatList
          ...
          testID="raw-division-mapping-list"
        />
      </View>
