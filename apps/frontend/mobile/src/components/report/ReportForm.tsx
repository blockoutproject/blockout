...
import { ReportType, type ReportCreatedView } from '@/src/types/Report';
...
import { createMobileReport } from '@/src/api/generated/mobile-gateway/endpoints/mobile-reports/mobile-reports';
import {
  CreateMobileReportBody,
  CreateMobileReportResponse,
} from '@/src/api/generated/mobile-gateway/schemas/mobile-reports/mobile-reports.zod';
...
export type ReportFormProps = {
  ...
  onSuccess: (created: ReportCreatedView) => void;
  ...
  const formik = useFormik<FormValues>({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const payload = CreateMobileReportBody.shape.data.parse({
          type: values.type,
          title: values.title.trim(),
          description: values.description?.trim(),
          appVersion: CURRENT_APP_VERSION ?? undefined,
          userId: context?.userId ?? customUser?.id ?? undefined,
        ...
        const created = CreateMobileReportResponse.parse(
          await createMobileReport({
            data: payload,
            images: images.map(toOrvalMultipartFile),
          }),
        );
        ...
        <View style={[styles.card, { backgroundColor: theme.surface }]}>
          ...
          >
            ...
            >
              ...
              />
              <Text style={[styles.addBtnText, { color: theme.textInactive }]}>
                Ajouter
              </Text>
            </TouchableOpacity>
          </ScrollView>
