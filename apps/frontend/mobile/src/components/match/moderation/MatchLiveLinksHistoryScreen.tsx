...
import { deleteMobileMatchLiveLink } from '@/src/api/generated/mobile-gateway/endpoints/mobile-match-live-links/mobile-match-live-links';
import {
  approveMobileMatchLiveLink,
  reactivateMobileMatchLiveLink,
  rejectMobileMatchLiveLink,
} from '@/src/api/generated/mobile-gateway/endpoints/mobile-match-moderation/mobile-match-moderation';
import {
  DeleteMobileMatchLiveLinkParams,
  DeleteMobileMatchLiveLinkResponse,
} from '@/src/api/generated/mobile-gateway/schemas/mobile-match-live-links/mobile-match-live-links.zod';
import {
  ApproveMobileMatchLiveLinkParams,
  ApproveMobileMatchLiveLinkResponse,
  ReactivateMobileMatchLiveLinkParams,
  ReactivateMobileMatchLiveLinkResponse,
  RejectMobileMatchLiveLinkParams,
  RejectMobileMatchLiveLinkResponse,
...
> = ({ match }) => {
  ...
  const links = useMemo<MatchLiveLinkDTO[]>(() => data ?? [], [data]);
  ...
  const handleApprove = useCallback(
    async (link: MatchLiveLinkDTO) => {
      await handleAction(async () => {
        const path = ApproveMobileMatchLiveLinkParams.parse({
          id: link.id,
        });
        const response = await approveMobileMatchLiveLink(path.id);
        ApproveMobileMatchLiveLinkResponse.parse(response);
  ...
  const handleReject = useCallback(
    async (link: MatchLiveLinkDTO) => {
      await handleAction(async () => {
        const path = RejectMobileMatchLiveLinkParams.parse({
          id: link.id,
        });
        const response = await rejectMobileMatchLiveLink(path.id);
        RejectMobileMatchLiveLinkResponse.parse(response);
  ...
  const handleDeleteActive = useCallback(
    async (_link: MatchLiveLinkDTO) => {
      await handleAction(async () => {
        const path = DeleteMobileMatchLiveLinkParams.parse({
          matchId: match.id,
        });
        const response = await deleteMobileMatchLiveLink(path.matchId);
        DeleteMobileMatchLiveLinkResponse.parse(response);
      }, 'Impossible de supprimer ce lien, réessaie.');
    },
    [match.id, handleAction],
  );

  const handleReactivate = useCallback(
    async (link: MatchLiveLinkDTO) => {
      await handleAction(async () => {
        const path = ReactivateMobileMatchLiveLinkParams.parse({
          id: link.id,
        });
        const response = await reactivateMobileMatchLiveLink(path.id);
        ReactivateMobileMatchLiveLinkResponse.parse(response);
