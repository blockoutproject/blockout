...
function hasCoord(lat: number | null | undefined, lng: number | null | undefined): lat is number {
...
const ClubMapCard: React.FC<Props> = ({club}) => {
      ...
      features: [
        {
          ...
          geometry: {
            type: "Point" as const,
            coordinates: [club.longitude as number, club.latitude as number] as [number, number],
          },
        },
      ],
    };
  ...
  const images = useMemo(() => {
    ...
    const logoUrl = (club as any).logoUrl as string | undefined;
  ...
  const initialCamera = useMemo(() => {
    ...
    return {center: [club.longitude as number, club.latitude as number] as [number, number], zoom: 12};
