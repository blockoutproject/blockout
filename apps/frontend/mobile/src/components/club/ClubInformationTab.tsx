...
const ClubInformationsTab: React.FC<ClubInformationsTabProps> = ({club, scrollY}) => {
  const theme = useAppTheme();
  const insets = useSafeAreaInsets();

  const websiteDisplay = useMemo(() => {
    return club?.website ? String(club.website).replace(/^https?:\/\//, "") : null;
  }, [club?.website]);
  ...
  return (
    ...
    >
      <InfoCard title="Coordonnées">
        ...
        <InfoRow icon="map-marker-outline" label="Adresse" value={(club as any).address ?? null}/>
