...
import { registerMobilePushToken } from '../api/generated/mobile-gateway/endpoints/mobile-notifications/mobile-notifications';
import {
  RegisterMobilePushTokenBody,
  RegisterMobilePushTokenParams,
  RegisterMobilePushTokenResponse,
} from '../api/generated/mobile-gateway/schemas/mobile-notifications/mobile-notifications.zod';
...
export async function registerPushTokenOnBackend(
  ...
  const path = RegisterMobilePushTokenParams.parse({ userId });
  const payload = RegisterMobilePushTokenBody.parse({
    expoPushToken,
    platform: platformToEnum(),
    deviceId,
  });

  const response = await registerMobilePushToken(path.userId, payload);
  RegisterMobilePushTokenResponse.parse(response);
}
