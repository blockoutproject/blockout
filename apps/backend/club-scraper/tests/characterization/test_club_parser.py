...
def test_parses_the_complete_ffvb_club_shape() -> None:
    ...
    club = parse_club_page(_fixture("club_complete.html"), "club-1")

    assert club is not None
    assert club.identifier == "club-1"
    assert club.raw_name == "BLOCKOUT PARIS"
    assert club.name == "BLOCKOUT PARIS"
    assert club.phone_number == "06 01 02 03 04"
    assert club.email == "contact@example.invalid"
    assert club.website == "https://club.example.invalid"
    assert club.postal_code == "75001"
...
def test_parses_the_duplicated_postal_code_fallback() -> None:
    ...
    club = parse_club_page(_fixture("club_fallback_address.html"), "club-2")

    assert club is not None
    assert club.raw_name == "VOLLEY TEST"
    assert club.postal_code == "69001"
...
def test_returns_a_partial_club_when_provider_fields_are_missing() -> None:
    ...
    club = parse_club_page(_fixture("club_missing_fields.html"), "club-3")

    assert club is not None
    assert club.identifier == "club-3"
    assert club.raw_name is None
    assert club.name is None
    assert club.city == ""
    assert club.postal_code is None
...
def test_returns_none_when_beautiful_soup_cannot_parse(monkeypatch) -> None:
    ...
    from blockout_club_scraper.infrastructure.ffvb import parser
    ...
    result = parse_club_page("<html>", "club-4")
