...
public class PoolApplicationService implements PoolService {
    ...
    @Override @Transactional(readOnly = true)
    public PoolView getPoolById(Long id) { return toView(loadPool(id)); }
    ...
    public void deactivatePool(Long id) {
        PoolEntity pool = loadPool(id); pool.setActive(false); poolRepository.saveAndFlush(pool);
    ...
    public PoolView incrementFollowersCount(Long poolId, Long userId) {
        PoolEntity pool = loadPool(poolId); pool.setFollowersCount(pool.getFollowersCount() + 1L);
        return toView(poolRepository.saveAndFlush(pool));
    }

    @Override @Transactional
    public PoolView decrementFollowersCount(Long poolId, Long userId) {
        PoolEntity pool = loadPool(poolId); pool.setFollowersCount(Math.max(0L, pool.getFollowersCount() - 1L));
        return toView(poolRepository.saveAndFlush(pool));
    }
