...
public class RabbitMQConfig {
  ...
  public static final String MATCH_LIVE_LINK_CREATED_QUEUE =
      "match.live-link-created.queue.notifications";
  public static final String MATCH_LIVE_LINK_CREATED_DLQ =
      "match.live-link-created.queue.notifications.dlq";
  ...
  public Binding bindUserFollowQueueTeams(
      Queue userFollowQueueTeams, TopicExchange userFollowExchange) {
    return BindingBuilder.bind(userFollowQueueTeams).to(userFollowExchange).with(RK_TEAM_FOLLOW);
  }

  @Bean
  public Binding bindUserFollowQueuePools(
      Queue userFollowQueuePools, TopicExchange userFollowExchange) {
    return BindingBuilder.bind(userFollowQueuePools).to(userFollowExchange).with(RK_POOL_FOLLOW);
  }

  @Bean
  public Binding bindTeamFollowDlq(Queue teamFollowDlq, TopicExchange userFollowDlqExchange) {
    return BindingBuilder.bind(teamFollowDlq).to(userFollowDlqExchange).with(RK_TEAM_FOLLOW_DLQ);
  }

  @Bean
  public Binding bindPoolFollowDlq(Queue poolFollowDlq, TopicExchange userFollowDlqExchange) {
    return BindingBuilder.bind(poolFollowDlq).to(userFollowDlqExchange).with(RK_POOL_FOLLOW_DLQ);
  }

  @Bean
  public Binding bindMatchFinishedQueue(
      Queue matchFinishedQueue, TopicExchange entityLifecycleExchange) {
  ...
  @Bean
  public Binding bindMatchFinishedDlq(
      Queue matchFinishedDlq, TopicExchange entityLifecycleDlqExchange) {
  ...
  public Binding bindMatchLiveLinkCreatedQueue(
      Queue matchLiveLinkCreatedQueue, TopicExchange entityLifecycleExchange) {
  ...
  @Bean
  public Binding bindMatchLiveLinkCreatedDlq(
      Queue matchLiveLinkCreatedDlq, TopicExchange entityLifecycleDlqExchange) {
