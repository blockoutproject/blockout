...
public class NotificationOrchestratorApplicationService {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(NotificationOrchestratorApplicationService.class);
  ...
  public void handleMatchFinished(
    ...
    ObjectNode baseMetadata =
        objectMapper.createObjectNode().put("divisionId", content.divisionId());

    // Réservation typée MATCH_FINISHED
    List<Long> reservedUserIds =
        notificationSendService.reservePendingForMatchFinished(matchId, teamIdA, teamIdB, poolId);
  ...
  /** Nouveau lien de live (avant / pendant match) -> notif "MATCH_LIVE_LINK_CREATED". */
  public void handleMatchLiveLinkCreated(Long matchId, Long teamIdA, Long teamIdB, Long poolId) {
    ...
    List<Long> reservedUserIds =
        notificationSendService.reservePendingForMatchLiveLinkCreated(
            matchId, teamIdA, teamIdB, poolId);
  ...
  private void processNotificationPipeline(
    ...
    logger.info(
        "Starting token resolution & push",
    ...
    for (int from = 0; from < reservedUserIds.size(); from += RESOLVE_PAGE_SIZE, pageIndex++) {
      ...
      if (page.getTokensByUser() != null && !page.getTokensByUser().isEmpty()) {
        for (Map.Entry<Long, List<String>> e : page.getTokensByUser().entrySet()) {
          ...
          List<String> tokens =
              e.getValue() == null ? List.of() : e.getValue().stream().distinct().toList();
      ...
      if (!usersFailed.isEmpty()) {
        notificationSendService.markFailed(
            matchId, usersFailed, "EXPO_SEND_ERROR", "See logs for details");
      }
  ...
  private ResolvedContent resolveFinishedContent(
    ...
    try {
      ...
      logger.warn(
          "Failed to resolve pool name/logo",
  ...
  private ResolvedContent resolveLiveLinkContent(
      Long matchId, Long teamIdA, Long teamIdB, Long poolId) {
    ...
    try {
      ...
      logger.warn(
          "Failed to resolve pool name for live link",
    ...
    try {
      ...
      logger.warn(
          "Failed to resolve team names for live link",
    ...
    String body =
        String.format(
            "🔴 Le match %s vs %s est en live ! Clique pour regarder", teamAName, teamBName);
