...
async def add_or_update_team(session: aiohttp.ClientSession, team: Team, existing_team: Optional[Team]) -> Team:
    ...
    if existing_team:
        ...
        for field in ['clubId', 'divisionId', 'format', 'gender', 'rawName']: # A revoir car pas forcement bon, ici on cherche à modifer les champs uniques ...
...
async def find_team_by_name_in_division_format_gender_season(
    ...
    """
    1) Récupère toutes les équipes correspondant à (divisionId, format, gender).
    2) Filtre pour trouver celle dont rawName correspond à 'searched_name' (insensible à la casse).
    3) Retourne la première correspondante ou None si introuvable.
    """
    teams = await get_teams(session, divisionId, format, gender, season)
    if not teams:
        return None

    # On peut standardiser les noms (lowercase, trim, etc.) selon ta logique
    searched_lower = rawName.strip().lower()
