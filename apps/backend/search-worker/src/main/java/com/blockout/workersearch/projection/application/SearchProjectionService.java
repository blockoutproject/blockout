...
public class SearchProjectionService {
  ...
  public void upsertClubs(List<ClubProjectionSource> clubs) {
    ...
    clubs.forEach(club -> indexTeams(projectionCache.findTeamsByClub(club.id())));
  }
  ...
  private void indexTeams(List<TeamProjectionSource> teams) {
    LOGGER.info(
  ...
  public void deactivateTeam(Long teamId) {
    ...
    projectionCache.removeTeam(teamId);
  }
  ...
  private TeamSearchProjection toTeamProjection(TeamProjectionSource team) {
    ...
    String logoUrl =
        StringUtils.isNotBlank(team.logoUrl())
            ? team.logoUrl()
