...
public class NotificationApplicationService {
  ...
  public NotificationPageView getNotifications(int page, int size) {
    ...
    List<NotificationItemView> rawItems =
        (base == null || base.notifications() == null)
            ? Collections.emptyList()
            : base.notifications();

    logger.debug(
        "Base notifications received",
    ...
    logger.debug(
        "Extracted divisionIds from metadata",
        keyValue("action", "extract_division_ids"),
        keyValue("division_ids_count", divisionIds.size()));

    List<Long> toResolve =
        divisionIds.stream().filter(id -> !divisionLogoCache.containsKey(id)).toList();
    ...
    for (NotificationItemView n : rawItems) {
      String divisionLogoUrl =
          extractDivisionIdSafely(n.metadata()).map(divisionLogoCache::get).orElse(null);
    ...
    logger.debug(
        "Built enriched notifications page",
  ...
  public UnreadCountView getUnreadNotificationsCount() {
    logger.info("Fetching unread notifications count", keyValue("action", "fetch_unread_count"));
