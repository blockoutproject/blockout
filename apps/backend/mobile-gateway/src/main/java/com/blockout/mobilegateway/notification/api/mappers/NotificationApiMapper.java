...
public class NotificationApiMapper {
   ...
   */
  public NotificationPageResponse toResponse(NotificationPageView source) {
   ...
   */
  private NotificationResponse toResponse(NotificationItemView source) {
    return new NotificationResponse(
            source.id(),
            source.userId(),
            NotificationTypeEnum.valueOf(source.type().name()),
            source.title(),
            source.body(),
            source.isRead(),
            source.isOpened(),
        ...
        .metadata(
            source.metadata() == null
                ...
                : objectMapper.convertValue(
                    source.metadata(), new TypeReference<Map<String, Object>>() {}))
  ...
  public RegisterPushTokenCommand toCommand(RegisterPushTokenRequest source) {
    return new RegisterPushTokenCommand(
        source.getExpoPushToken(),
        DevicePlatform.valueOf(source.getPlatform().name()),
        source.getDeviceId());
  }
   ...
   */
  public UnreadCountResponse toResponse(UnreadCountView source) {
