...
public class ClubInternalClient {
  ...
  public ClubView getClubById(String id) {
    String url =
        UriComponentsBuilder.fromUriString(baseUrl()).pathSegment(id).build().toUriString();

    ResponseEntity<ClubInternalResponse> response =
        internalApiClient.get(url, ClubInternalResponse.class);
  ...
  public String getClubLogoUrl(String id) {
    String url =
        UriComponentsBuilder.fromUriString(baseUrl()).pathSegment(id, "logo").build().toUriString();

    ResponseEntity<String> response = internalApiClient.get(url, String.class);
    String body = response.getBody();

    return (response.getStatusCode() == HttpStatus.NO_CONTENT || body == null || body.isBlank())
        ? null
        : body;
  }

  @Caching(
      put = {@CachePut(value = "clubById", key = "#id")},
      evict = {@CacheEvict(value = "clubLogoById", key = "#id")})
  public ClubView updateClub(String id, UpdateClubCommand command, MultipartFile image) {
    ...
    ResponseEntity<ClubInternalResponse> response =
        internalApiClient.putMultipart(url, body, ClubInternalResponse.class);
    return contractMapper.toResponse(response.getBody());
  }
}
