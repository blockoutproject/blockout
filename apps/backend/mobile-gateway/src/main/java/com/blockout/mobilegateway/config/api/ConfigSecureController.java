...
public class ConfigSecureController implements ConfigSecureApi {
  ...
  public ResponseEntity<AppStatusResponse> updateAppStatus(UpdateAppStatusRequest request) {
    return ResponseEntity.ok(
        mapper.toResponse(configService.updateAppStatus(mapper.toCommand(request))));
  }
  ...
  public ResponseEntity<DivisionResponse> createDivision(String data, MultipartFile image) {
    ...
    DivisionResponse created =
        mapper.toResponse(configService.createDivision(mapper.toCommand(request), image));
    URI location =
  ...
  public ResponseEntity<DivisionResponse> updateDivision(
      Long id, String data, MultipartFile image) {
    UpsertDivisionRequest request = read(data, UpsertDivisionRequest.class);
    return ResponseEntity.ok(
        mapper.toResponse(configService.updateDivision(id, mapper.toCommand(request), image)));
  }
  ...
  public ResponseEntity<RawDivisionMappingResponse> createRawDivisionMapping(
    ...
    RawDivisionMappingResponse created =
        mapper.toResponse(configService.createRawDivisionMapping(mapper.toCommand(request)));
    URI location =
        ServletUriComponentsBuilder.fromCurrentRequest()
            .path("/{id}")
            .buildAndExpand(created.getId())
  ...
  public ResponseEntity<LegalDocumentResponse> updateLegalDocument(
    ...
    return ResponseEntity.ok(
        mapper.toResponse(configService.updateLegalDocument(type, mapper.toCommand(request))));
  }
  ...
  public ResponseEntity<List<RawDivisionMappingResponse>> listRawDivisions(
    ...
    return ResponseEntity.ok(
        configService.listRawDivisionMappings(leagueCode, season).stream()
  ...
  public ResponseEntity<RawDivisionMappingResponse> updateRawDivision(
    ...
    return ResponseEntity.ok(
        mapper.toResponse(configService.updateRawDivisionMapping(id, mapper.toCommand(request))));
  }
  ...
  public ResponseEntity<List<ScraperStatusResponse>> listScraperStatuses() {
    return ResponseEntity.ok(
        configService.listScraperStatuses().stream().map(mapper::toResponse).toList());
  }
