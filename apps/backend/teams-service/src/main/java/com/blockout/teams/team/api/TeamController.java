...
public class TeamController implements TeamApi {
  ...
  public ResponseEntity<List<TeamInternalResponse>> listTeams(
    ...
    return ResponseEntity.ok(
        teamService
            .findTeams(
                divisionId,
                mapper.toFormat(format),
                mapper.toGender(gender),
                season,
                clubId,
                ids,
                active)
  ...
  public ResponseEntity<TeamInternalResponse> updateTeam(
    ...
    return ResponseEntity.ok(
        mapper.toInternalResponse(teamService.updateTeam(id, mapper.toCommand(request, image))));
  }
  ...
  public ResponseEntity<TeamInternalResponse> incrementFollowers(Long teamId, Long userId) {
    return ResponseEntity.ok(
        mapper.toInternalResponse(teamService.incrementFollowersCount(teamId, userId)));
  }
  ...
  public ResponseEntity<TeamInternalResponse> decrementFollowers(Long teamId, Long userId) {
    return ResponseEntity.ok(
        mapper.toInternalResponse(teamService.decrementFollowersCount(teamId, userId)));
  }
