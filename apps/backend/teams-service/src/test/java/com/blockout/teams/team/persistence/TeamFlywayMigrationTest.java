...
class TeamFlywayMigrationTest {

    @Test
    void migrationAddsRevisionAndAllowsCanonicalOnlyOutboxRows() throws Exception {
        String url = "jdbc:h2:mem:mrg441;MODE=PostgreSQL;DB_CLOSE_DELAY=-1";
        try (var connection = DriverManager.getConnection(url, "sa", "");
                var statement = connection.createStatement()) {
            statement.executeUpdate("CREATE TABLE teams (id BIGINT PRIMARY KEY)");
        ...
        try (var connection = DriverManager.getConnection(url, "sa", "");
                var statement = connection.createStatement()) {
            statement.executeUpdate("INSERT INTO teams (id) VALUES (1)");
            try (var result = statement.executeQuery("SELECT revision FROM teams WHERE id = 1")) {
            ...
            statement.executeUpdate("""
                    INSERT INTO event_outbox (
                        ...
                        '%s', 'TEAM_PROJECTION_CHANGED', '2.0.0', 'teams-service', 'team:1', 0,
                        CURRENT_TIMESTAMP, 'entity.lifecycle.exchange', NULL, NULL, NULL,
                        TRUE, 'team.projection-changed.v2', JSON '{}'
                    )
