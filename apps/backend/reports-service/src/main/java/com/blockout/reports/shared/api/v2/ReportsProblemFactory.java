package com.blockout.reports.shared.api.v2;
...
/** Creates canonical reports-service Problem Details with stable request identifiers. */
@Component
public class ReportsProblemFactory {

    public static final String REQUEST_ID_HEADER = "X-Request-Id";
    public static final String REQUEST_ID_ATTRIBUTE = ReportsProblemFactory.class.getName() + ".requestId";
    private static final int MAX_REQUEST_ID_LENGTH = 255;
    private static final Logger LOGGER = LoggerFactory.getLogger(ReportsProblemFactory.class);

    /** Creates one canonical Problem Details response. */
    public ResponseEntity<ProblemDetail> response(
        ...
        ProblemDetail problem = new ProblemDetail(status.getReasonPhrase(), status.value(), code)
                ...
                .requestId(requestId);
        LOGGER.info("Reports v2 problem",
                keyValue("problemCode", code), keyValue("status", status.value()), keyValue("requestId", requestId));
    ...
    /** Reuses one safe caller identifier or creates a new identifier. */
    ...
    /** Accepts only bounded non-blank request identifiers. */
