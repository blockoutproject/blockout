...
@SpringBootTest(classes = RabbitUserFollowIntegrationTest.TestApplication.class)
@Testcontainers
@DisplayName("Rabbit user follow integration")
class RabbitUserFollowIntegrationTest {

  private static final String TEST_QUEUE = "user.follow.queue.integration";
  ...
  @Autowired private RabbitUserFollowPublisher publisher;
  ...
  void declareConsumerQueue() {
    ...
    rabbitAdmin.declareBinding(
        BindingBuilder.bind(queue)
            .to(new TopicExchange(RabbitMQConfig.USER_FOLLOW_EXCHANGE))
            .with("team.follow"));
  }
  ...
  @Test
  @DisplayName("publishes serialized follow events through the configured exchange")
  void publishesSerializedFollowEvents() {
    publisher.publish(7L, EntityType.TEAM, 11L, FollowEventType.CREATED);
    ...
    assertThat(new String(message.getBody(), StandardCharsets.UTF_8))
        .contains("\"userId\":7")
        .contains("\"entityType\":\"TEAM\"")
        .contains("\"entityId\":11")
  ...
  @Import({RabbitMQConfig.class, RabbitUserFollowPublisher.class})
