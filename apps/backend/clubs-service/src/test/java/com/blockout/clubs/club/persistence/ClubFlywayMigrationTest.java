...
class ClubFlywayMigrationTest {

    @Test
    void migrationAddsRevisionAndAllowsCanonicalOnlyOutboxRows() throws Exception {
        String url = "jdbc:h2:mem:mrg440;MODE=PostgreSQL;DB_CLOSE_DELAY=-1";
        ...
        try (var connection = DriverManager.getConnection(url, "sa", "");
                var statement = connection.createStatement()) {
            statement.executeUpdate("INSERT INTO clubs (id, raw_name, name) VALUES ('club-1', 'Raw', 'Club')");
            try (var result = statement.executeQuery("SELECT revision FROM clubs WHERE id = 'club-1'")) {
            ...
            statement.executeUpdate("""
                    INSERT INTO event_outbox (
                        ...
                        '%s', 'CLUB_PROJECTION_CHANGED', '2.0.0', 'clubs-service', 'club:club-1', 0,
                        CURRENT_TIMESTAMP, 'entity.lifecycle.exchange', NULL, NULL, NULL,
                        TRUE, 'club.projection-changed.v2', JSON '{}'
                    )
