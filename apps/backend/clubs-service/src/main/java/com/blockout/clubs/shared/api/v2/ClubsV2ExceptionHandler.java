...
@RestControllerAdvice(assignableTypes = {ClubV2Controller.class, ClubLogoV2Controller.class})
@RequiredArgsConstructor
public class ClubsV2ExceptionHandler {

    private final ClubsProblemFactory problems;

    @ExceptionHandler(ClubNotFoundException.class)
    public ResponseEntity<ProblemDetail> handleNotFound(
            ClubNotFoundException exception,
            HttpServletRequest request) {
        return problems.response(HttpStatus.NOT_FOUND, "club_not_found", exception.getMessage(), request);
    }
