...
public class ClubController implements ClubApi {
  ...
  public ResponseEntity<List<ClubInternalResponse>> listClubs(List<String> ids, Boolean active) {
    return ResponseEntity.ok(
        clubService.findClubs(ids, active).stream().map(mapper::toInternalResponse).toList());
  }
  ...
  public ResponseEntity<ClubInternalResponse> updateClub(
      String id, String data, MultipartFile image) {
    UpdateClubInternalRequest request = readData(data, UpdateClubInternalRequest.class);
    return ResponseEntity.ok(
        mapper.toInternalResponse(clubService.updateClub(id, mapper.toCommand(request, image))));
  }
