...
**Architecture:** Each template mirrors only configuration read by its application, uses the monorepo Compose ports and database names for local dependencies, and uses explicit replacement values for external credentials. Developers copy the template to the local filename already loaded by the application: `.env.local` for Java and Expo, and `.env` or `.env.local` for Python according to the selected scraper argument.
...
- Create: `apps/backend/{clubs,competition,config,matches,mobile-gateway,notification,pools,reports,search,teams,users}-service/.env.example`
