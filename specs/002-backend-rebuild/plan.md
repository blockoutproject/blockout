...
POST `/api/v2/webhooks/revenuecat` verifies HMAC-SHA256 on timestamp+'.'+original bytes through a
controller-scoped Spring RequestBodyAdvice before conversion, with constant-time comparison and
300-second timestamp tolerance. Generated transport models describe the required provider fields.
