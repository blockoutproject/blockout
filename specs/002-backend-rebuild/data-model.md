...
One row, id=1, generation=2. Application readiness reads the supported generation and required jobs columns. An incompatible or missing schema fails readiness. Additive compatible migrations retain generation.
...
The first identity baseline uses generation=2, including the existing jobs tables plus:
