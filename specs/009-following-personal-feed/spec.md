...
1. **A18** — **Étant donné** plusieurs relations de suivi et des matchs connus, **lorsque** le fil est consulté, **alors** les cas suivants s’appliquent. (FR-016, FR-018, FR-019, FR-021)

   - **Union et déduplication** : Suivre les deux équipes et leur poule présente une seule fois chaque match actuellement consultable de la saison sélectionnée.
   - **Match retiré avec résultat** : Même si l’équipe reste consultable dans une autre poule, un match retiré de son ancien calendrier selon F02 est absent du fil et de sa fiche publique ; son résultat conservé en interne ne le rend pas affichable. Le suivi de l’ancienne poule ne contourne pas le retrait.
   - **Calendrier vide confirmé** : La première observation vide qualifiée conserve les matchs encore autorisés ; la seconde confirmation planifiée consécutive selon F01/F02 les retire du fil, sans supprimer les relations de suivi. Les équipes devenues masquées disparaissent aussi des listes.
   - **Panne et réapparition** : Un échec de téléchargement seul ne retire aucun match. Une réapparition autorisée retrouve les identités et suivis conservés sans doublon et sans attendre deux nouvelles collectes, sous réserve de la saison choisie et des autres restrictions.
...
- **FR-018** : Le fil DOIT sélectionner uniquement les matchs actuellement consultables selon F02/F03 de la saison sportive choisie, liés à au moins une équipe ou une poule suivie applicable. Une équipe sélectionne ses rencontres dans ses participations consultables ; une poule sélectionne ses rencontres consultables. Une équipe ou une poule suivie ne rend jamais consultable un match retiré, même avec un résultat conservé en interne. Le retrait total après deux confirmations de calendrier vide suit F01/F02 ; une panne de lecture ne le déclenche pas. Toutes les saisons enlève uniquement la restriction saisonnière. Sans suivi applicable, aucun calendrier général de substitution n’est présenté comme personnel.
...
- **SC-004** : A18–A21 présentent chaque match actuellement admissible une seule fois et le conservent tant qu’un suivi applicable le sélectionne et que F02/F03 l’autorisent. Aucun suivi ou résultat conservé ne réexpose un match retiré ; la première confirmation vide et les pannes ne produisent pas de retrait total, la seconde confirmation qualifiée l’autorise, et la réapparition respecte les restrictions indépendantes. Tous les cas temporels utilisent F03. (FR-016, FR-018–FR-019, FR-021–FR-023)
...
| [F07](../011-notifications-delivery/spec.md)                                              | Consomme les relations confirmées ; possède éligibilité, préférences et livraison des notifications. La saison du fil reste un filtre d’affichage.                                                                    |
...
| [F10](../012-reports-feature-suggestions/spec.md) / [F13](../003-shared-quality/spec.md)  | Signalement contextualisé, minimisation, performance, accessibilité, fraîcheur, erreurs et diagnostic. Aucun nouveau seuil F06.                                                                                       |
